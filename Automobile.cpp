...
void Automobile::addFuel(double _liters)
{
	...
	char message[64];
	sprintf(message, "The %s %d %s %s is full of gas. Discarding the rest...", colour.c_str(), year, make.c_str(), model.c_str());
...
void Automobile::drive(double _distance)
{
    ...
    if(fuelInTank < 0) {
        fuelInTank = 0;
	char message[64];
	sprintf(message, "The %s %d %s %s has no gas left in the tank", colour.c_str(), year, make.c_str(), model.c_str());
